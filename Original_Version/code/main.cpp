...
char testVarianceFolder[MAX_WORD_LEN] = ".\\test_med_down_15";
...
inline void X_DirectedGradientGeneration(const Mat &Gray_openingGaussian , Mat &Grad_X_Thresh_Pop){	
	...
	threshold(ABS_Grad_X, Grad_X_Thresh_Pop, thre_populationGray, 255, cv::THRESH_BINARY);
}
...
inline void Y_DirectedGradientGeneration(const Mat &Gray_openingGaussian , Mat &Grad_Y_Thresh_Pop){	
	...
	threshold(ABS_Grad_Y, Grad_Y_Thresh_Pop, thre_populationGray, 255, cv::THRESH_BINARY);
}
...
//	drawContours( GradsConvexHull, hull, 0, Scalar(255 , 255 , 255), cv::FILLED);//Fill the range in the convex hull	
														...
														, const int &iris_x_regionLeft , const int &iris_x_regionRight)
{
	...
	drawContours( ValleyPeakField, hull, 0, Scalar(255 , 255 , 255), cv::FILLED);//Fill the range in the convex hull	
																 ...
																 , vector<float> &vec_Deriv_GPFv_x)
{
	...
	for(int i=0;i<GPFh_y.rows;++i){
		GPFh_y.at<float>(i , 0) = IPFh_y.at<float>(i , 0);
	}
	//for(int i=0;i<GPFh_y.rows;++i){
...
{	
	...
	//Delete those Feature Points Out of Legt & Right Region
	for(int i=0;i<feature_point_Limbus.size();++i){
...
int main(int argc , char *argv[]){	
		...
		sprintf(video_input_file, "C:\\Users\\Coslate\\File_Change\\Lab421\\EyeGazeTrackingSystem_speedUp_vf\\EyeGazeTrackingSystem_speedUp_vf\\EyeGazeTrackingSystem_speedUp_vf\\Test_Mean_Variance_Data\\med_down_%d\\med_down_%d.avi", video_num, video_num);
