...
//Testing File
char thesisOutFileName[MAX_WORD_LEN];
char analysisGazeOutputDir [MAX_WORD_LEN]= "F:\\LAB_new\\LAB\\Eye_track_code\\Eye_Pos_Track\\EyeGazeTrackingSystem_v2\\EyeGazeTrackingSystem_v2\\Result\\Gaze_Result_Polynomial";
char thesisImageOutDir[MAX_WORD_LEN] = "C:\\Users\\sychien\\Desktop\\Oral\\Thesis\\Fig";
...
std::vector<std::vector<cv::Point>> testEyeInputPts;
std::vector<cv::Point> testOneDstEyePts;
...
int irisCenter_Intensity_Color_ExchangeRegionLimit = 15;
...
int iris_maskModel_refreshFrame_AfterGet = 30;
float iris_colorModelValidTestingIrisRate_initial = 40;//40
float iris_colorModelIrisRate_pixelInOthersOne = 0.45;// IrisRegionValidTestSize*45%
...
inline bool FindMAXConnextedComponent(const Mat& Src , vector<Point> &max_contours , Mat &Contour_dst){
	...
	findContours( Contour_src, contours, hierarchy, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_SIMPLE );
	...
	if( !contours.empty() && !hierarchy.empty() ){
		...
		drawContours( Contour_dst, contours, idx, Scalar(255 , 255 , 255), CV_FILLED, 8, hierarchy );		
	}	
	return true;
}
																	...
																	, float OffFactor)
{
	...
	if( !contours.empty() && !hierarchy.empty() ){
		...
		for( ; idx >= 0; idx = hierarchy[idx][0] ){
			...
			drawContours( OutMask, contours, idx, color, CV_FILLED, 8, hierarchy );
		}
...
inline void HSVProjectionMethod(const Mat &src , int &maximal_y , int &maximal_x){
	...
	Point minLoc;
	Point maxLoc;
	reduce(src, colSum, 1, CV_REDUCE_SUM , CV_32FC1);
	...
	double min_row;
	double max_row;
	reduce(src, rowSum, 0, CV_REDUCE_SUM , CV_32FC1);
...
inline void GrayProjectionMethod(const Mat &src,int &minimal_y , int &minimal_x){
	...
	Point minLoc;
	Point maxLoc;
	reduce(src, colSum, 0, CV_REDUCE_SUM , CV_32FC1);
	minMaxLoc( colSum, &min_col, &max_col, &minLoc, &maxLoc , Mat());
	minimal_x = minLoc.x;
	...
	double min_row;
	double max_row;
	reduce(src, rowSum, 1, CV_REDUCE_SUM , CV_32FC1);
	minMaxLoc( rowSum, &min_row, &max_row, &minLoc, &maxLoc , Mat());
	minimal_y = minLoc.y;
...
inline void DrawEllipse(const Mat &image_orig , Mat &dst  ,const float* ellipse_param 
    ...
    ellipse(dst,CircleCenter,EllipseAxes,RotateAngle,
                 StartDrawingAngle,StopDrawingAngle,Color,Thickness,CV_AA,Shift);//Draw ellipse
...
inline void DrawPrabolaVer2(Mat &dst  , const float *const parabola_para , const Mat &OriginalFrame){
	dst = OriginalFrame.clone();
	vector<Point> pt;
	 for (int x = 0; x < FRAMEW; ++x)
     {
         float y = parabola_para[0] * double(x) * double(x) + parabola_para[1] * double(x) + parabola_para[2];
         pt.push_back(Point(x, int(y)));
     }     
     for (int i = 0; i < pt.size()-1; ++i)
     {
         line(dst, pt[i], pt[i+1], Scalar(200 , 225 , 100) , 20 ,CV_AA );//20
     }     
}

inline void DrawPrabolaVer3(Mat &dst  , const float *const parabola_para){	
	vector<Point> pt;
	 for (int x = 0; x < FRAMEW; ++x)
     {
         float y = parabola_para[0] * double(x) * double(x) + parabola_para[1] * double(x) + parabola_para[2];
     ...
     for (int i = 0; i < pt.size()-1; ++i)
     {
         line(dst, pt[i], pt[i+1], Scalar(200 , 225 , 100) , 20 ,CV_AA );
     }     
}
...
inline void ValleyPeakFieldConstruction_ver2(const int frame_number , const Mat &Eyelid){
	...
	cvtColor(Frame_wh, Framw_wh_HLS, CV_BGR2HLS );	
	...
	//int min_y , min_x;
	//GrayProjectionMethod(Preprocrss_Blur_Log , Gray_Projection_Pupil_Center , min_y , min_x);//For displaying coarse pupil location 
	//GrayProjectionMethod(Preprocrss_Blur_Log , min_y , min_x);
	//time_b = omp_get_wtime();
	//printf("GrayProjectionMethod time = %f\n" , time_b - time_a);
...
//	vector<Point> feature_point_all;
//	vector<Point> feature_point_iris;
//	vector<Point> feature_point_eyelid;
//	Point  start_point(min_x , min_y);
//	Point  start_point_for_model(min_x , min_y);
//	//LimbusFeatureDetection(Preprocrss_Blur, feature_point_iris ,feature_point_eyelid , frame_number,1
//	//										, gradient_threshold, start_point, start_point_use_coarse_pupil_detection);
//	//LimbusFeatureDetection(Noise_reduce, Eyelid , feature_point_all, frame_number,1
////											, gradient_threshold, start_point, start_point_use_coarse_pupil_detection);
//
//	LimbusFeatureDetection(Preprocrss_Blur, feature_point_all , frame_number,3
//											, gradient_threshold, start_point, start_point_use_coarse_pupil_detection);
	//time_b = omp_get_wtime();
	//printf("LimbusFeatureDetection time = %f\n" , time_b - time_a);

	

	//-------------------Model Fitting---------------------//		
	//ModelFitting(feature_point_all, start_point_for_model , Size(Frame_wh.cols , Frame_wh.rows) , Ellipse);


	////imshow("Noise_Reduce" , Noise_reduce);
	//imshow("Preprocessing_Blur" , Preprocrss_Blur);
	////imshow("Preprocrss_Log" , Preprocrss_Blur_Log);
	//imshow("Preprocrss_Log_Normalize" , Preprocrss_Blur_Log_Normalize);	
	//imshow("L - TopHat(L)" , L_Tophat_Diff);		
	//imshow("HLS_tophat_L" , HLS_tophat);
	//imshow("Gray_Projection_Pupil_Center" , Gray_Projection_Pupil_Center);
	////imshow("H" , Hls_planes[0]);
	//imshow("L" , Hls_planes[1]);	
	//imshow("S" , Hls_planes[2]);	
}
...
inline void SaturationProjectionProcess(const Mat &Src ,int &posY , int &posX , const int &size_gaussian){	
	...
	threshold(S_openingGaussian, S_Otsu, 0, 255, CV_THRESH_BINARY | CV_THRESH_OTSU);
...
inline void MinimalGrayProjectionProcess(const Mat &Src ,int &posY , int &posX , int &size_gaussian){	
	...
	threshold(Inv_Src, GrayOtsu, 0, 255, CV_THRESH_BINARY | CV_THRESH_OTSU);
...
inline bool MinimalIrisColorProcess(const Mat &Src , Point &eyeCoarseCenter , int &size_gaussian 
													, Mat &IrisContour , vector<Point> &IrisContoursPoints , bool &getIrisContourPoints
													, float &irisContour_size)
{	
	Mat GrayGaussian;
	Mat GrayOtsu;				
	Mat Center_disp = Frame_wh.clone();	

	GaussianBlur( Src, GrayGaussian, Size(size_gaussian,size_gaussian) , 0);			
		
	threshold(GrayGaussian, GrayOtsu, 0, 255, CV_THRESH_BINARY | CV_THRESH_OTSU);
	...
	//-----------Can be delete later-----------//
	if(thesisClipImage){
		sprintf(thesisOutFileName , "%s\\MinimumIrisColor\\IrisMask_MaxCompon.jpg" , thesisImageOutDir);
		imwrite(thesisOutFileName , IrisContour);	
	}
	//----------------------------------------------//
	...
	if(showDetail){
		imshow("IrisMaskOtsu_color" , GrayOtsu);
	}

	if(thesisClipImage){
		sprintf(thesisOutFileName , "%s\\MinimumIrisColor\\IrisMask.jpg" , thesisImageOutDir);
		imwrite(thesisOutFileName , Src);		

		sprintf(thesisOutFileName , "%s\\MinimumIrisColor\\IrisMask_GauBlur.jpg" , thesisImageOutDir);
		imwrite(thesisOutFileName , GrayGaussian);	

		sprintf(thesisOutFileName , "%s\\MinimumIrisColor\\IrisMask_Otsu.jpg" , thesisImageOutDir);
		imwrite(thesisOutFileName , GrayOtsu);	
...
inline void GrayProjectionProcess(const Mat &src , Mat &dst , Mat &GaussianBlurOpening, const bool useGaussianBlur , int &minimal_y , int &minimal_x){
	...
	//threshold(GaussianBlurOpening, GaussianBlurOpening, 0, 255, CV_THRESH_BINARY | CV_THRESH_OTSU);
...
inline void ScleraMaskPreprocessing(const Mat &SrcMask , Mat &DstMask , const int &size_gaussian){
	...
	threshold(SrcMask_gaussian, SrcMask_gaussian, 0, 255, CV_THRESH_BINARY | CV_THRESH_OTSU);
...
inline void LineFilterRefinement(const vector<vector<Point> > &contours , vector<vector<Point> > &refined_contours
												 ...
												 , const bool &extremeRight , int &rescue_iris_x_regionLeft
												 , const bool &extremeDown , int &rescue_iris_x_regionRight
...
{		
	...
	vector<vector<Point>> leftGrads;
	vector<vector<Point>> rightGrads;
	vector<Point> leftgradsCenterVec;
	vector<Point> rightgradsCenterVec;
	for(int i = 0;i<contours.size();++i){
		int max_y = -INT_MAX;
		int min_y = INT_MAX;
	...
	if( !refined_contours.empty()){		
		for( int idx = 0; idx<refined_contours.size();++idx){			
			drawContours(RefinedGradX_EyeRegion , refined_contours, idx, Scalar(255 , 255 , 255), CV_FILLED, 8);
		}
	}	

	if(thesisClipImage){					
		sprintf(thesisOutFileName , "%s\\EyelidDetection\\RefinedGradX_EyeRegion.jpg" , thesisImageOutDir);
		imwrite(thesisOutFileName , RefinedGradX_EyeRegion);	
	}


	for(int i=0;i<refined_contours.size();++i){
		Point gradElementCenter;
		CenterCalculatUsingMoment(refined_contours[i] , gradElementCenter.x , gradElementCenter.y);	
		if(gradElementCenter.x>eyeCoarseCenter.x){
			rightgradsCenterVec.push_back(gradElementCenter);	
			rightGrads.push_back(refined_contours[i]);
		}else{
			leftgradsCenterVec.push_back(gradElementCenter);	
			leftGrads.push_back(refined_contours[i]);
		}		
	}

	//Caculate rescue_iris_x_regionLeft &rescue_iris_x_regionRight
	if(caculateIris_Mask_done && getEyeCoarseCenter && (extremeDown /*|| extremeLeft || extremeRight*/)){		
		int min_x = INT_MAX;
		int max_x = -INT_MAX;
		for(int i=0;i<IrisContoursPoints_byColor.size();++i){
			if(IrisContoursPoints_byColor[i].x>max_x){
				max_x = IrisContoursPoints_byColor[i].x;
			}
			if(IrisContoursPoints_byColor[i].x<min_x){
				min_x = IrisContoursPoints_byColor[i].x;
			}
		}
		rescue_iris_x_regionLeft = min_x;
		rescue_iris_x_regionRight = max_x;		
	}else if(extremeUp){
		//Left
		if(leftPeakDeriv_vx.size()==0){
			rescue_iris_x_regionLeft = 0;	
		}else if(leftPeakDeriv_vx.size()==1){
			rescue_iris_x_regionLeft = leftPeakDeriv_vx[0];
		}else{
			int firstHighest;
			int secondHighest;
			...
			if(firstHighest>200 && secondHighest>200){
				if(vx_LeftDeriv_peakDataAndPos[leftPeakDeriv_vx.size()-1].number < vx_LeftDeriv_peakDataAndPos[leftPeakDeriv_vx.size()-2].number){
					rescue_iris_x_regionLeft = vx_LeftDeriv_peakDataAndPos[leftPeakDeriv_vx.size()-2].number;
				}else{
					rescue_iris_x_regionLeft = vx_LeftDeriv_peakDataAndPos[leftPeakDeriv_vx.size()-1].number;
				}
			}else{
				rescue_iris_x_regionLeft = vx_LeftDeriv_peakDataAndPos[leftPeakDeriv_vx.size()-1].number;
			}		
		...
		if(rightPeakDeriv_vx.size()==0){
			rescue_iris_x_regionRight = IrisContour.cols - 1;
		}else if(rightPeakDeriv_vx.size()==1){
			rescue_iris_x_regionRight = rightPeakDeriv_vx[0];
		}else{
			int firstHighest;
			int secondHighest;
			...
			if(firstHighest>200 && secondHighest>200){
				if(vx_RightDeriv_peakDataAndPos[rightPeakDeriv_vx.size()-1].number < vx_RightDeriv_peakDataAndPos[rightPeakDeriv_vx.size()-2].number){
					rescue_iris_x_regionRight = vx_RightDeriv_peakDataAndPos[rightPeakDeriv_vx.size()-1].number;
				}else{
					rescue_iris_x_regionRight = vx_RightDeriv_peakDataAndPos[rightPeakDeriv_vx.size()-2].number;
				}
			}else{
				if(abs(vx_RightDeriv_peakDataAndPos[rightPeakDeriv_vx.size()-1].element 
					- vx_RightDeriv_peakDataAndPos[rightPeakDeriv_vx.size()-2].element)<20){
					if(vx_RightDeriv_peakDataAndPos[rightPeakDeriv_vx.size()-1].number<vx_RightDeriv_peakDataAndPos[rightPeakDeriv_vx.size()-2].number){
						rescue_iris_x_regionRight = vx_RightDeriv_peakDataAndPos[rightPeakDeriv_vx.size()-1].number;	
					}else{
						rescue_iris_x_regionRight = vx_RightDeriv_peakDataAndPos[rightPeakDeriv_vx.size()-2].number;	
		...
		rescue_iris_x_regionLeft = -INT_MAX;
		rescue_iris_x_regionRight = INT_MAX;
		float leftGrad_maxSize = -FLT_MAX;
		float rightGrad_maxSize = -FLT_MAX;	

		if(rightGrads.size()==0){
			rescue_iris_x_regionRight = FRAMEW - 1;
		}else{
			for(int i=0;i<rightGrads.size();++i){
				if(rightGrads[i].size()>rightGrad_maxSize){
					rightGrad_maxSize = rightGrads[i].size();
					rescue_iris_x_regionRight = rightgradsCenterVec[i].x;
				}
			}		
		}
		...
		if(leftGrads.size()==0){
			...
			for(int i=0;i<leftGrads.size();++i){
				if(leftGrads[i].size()>leftGrad_maxSize){
					leftGrad_maxSize = leftGrads[i].size();
					rescue_iris_x_regionLeft = leftgradsCenterVec[i].x;
				}		
			}
		}
		...
		if(leftGrads.size()==0){
			rescue_iris_x_regionLeft = iris_x_regionLeft;
		}else{
			rescue_iris_x_regionLeft = -INT_MAX;		
			for(int i=0;i<leftGrads.size();++i){			
				if(leftgradsCenterVec[i].x>rescue_iris_x_regionLeft){
					rescue_iris_x_regionLeft = leftgradsCenterVec[i].x;
				}
			}					
		}
		rescue_iris_x_regionRight = iris_x_regionRight;
		...
		if(rightGrads.size()==0){
			...
			rescue_iris_x_regionRight = INT_MAX;		
			for(int i=0;i<rightGrads.size();++i){			
				if(rightgradsCenterVec[i].x<rescue_iris_x_regionRight){
					rescue_iris_x_regionRight = rightgradsCenterVec[i].x;
				}
			}	
		}
	...
	}else if((leftGrads.size()==0)^(rightGrads.size()==0)){//Forming Convex Hull in One Size if the Other Side(right/left) Has no Grads
		if(leftGrads.size()==0 && rightGrads.size()!=0){			
			vector<int> refined_rightPeakDeriv_vx;			
			PeakRefinement(rightPeakDeriv_vx , rightGrads	, refined_rightPeakDeriv_vx);		
			if(refined_rightPeakDeriv_vx.size()==0){
				rescue_iris_x_regionRight = IrisContour.cols - 1;
			}else if(refined_rightPeakDeriv_vx.size()==1){
				rescue_iris_x_regionRight = refined_rightPeakDeriv_vx[0];
			}else{
				int firstHighest;
				int secondHighest;
				FindPeakof1D_Data* vx_RightDeriv_peakDataAndPos = new FindPeakof1D_Data [refined_rightPeakDeriv_vx.size()];
				for(int i=0;i<refined_rightPeakDeriv_vx.size();++i){
					vx_RightDeriv_peakDataAndPos[i].element = vec_Deriv_GPFv_x[refined_rightPeakDeriv_vx[i]];
					vx_RightDeriv_peakDataAndPos[i].number = refined_rightPeakDeriv_vx[i];
				}
				sort(vx_RightDeriv_peakDataAndPos , vx_RightDeriv_peakDataAndPos + refined_rightPeakDeriv_vx.size());	
				firstHighest = vx_RightDeriv_peakDataAndPos[refined_rightPeakDeriv_vx.size()-1].element;
				secondHighest = vx_RightDeriv_peakDataAndPos[refined_rightPeakDeriv_vx.size()-2].element;

				if(firstHighest>200 && secondHighest>200){
					if(vx_RightDeriv_peakDataAndPos[refined_rightPeakDeriv_vx.size()-1].number < vx_RightDeriv_peakDataAndPos[refined_rightPeakDeriv_vx.size()-2].number){
						rescue_iris_x_regionRight = vx_RightDeriv_peakDataAndPos[refined_rightPeakDeriv_vx.size()-1].number;
					}else{
						rescue_iris_x_regionRight = vx_RightDeriv_peakDataAndPos[refined_rightPeakDeriv_vx.size()-2].number;
					}
				}else{
					if(abs(vx_RightDeriv_peakDataAndPos[refined_rightPeakDeriv_vx.size()-1].element 
						- vx_RightDeriv_peakDataAndPos[refined_rightPeakDeriv_vx.size()-2].element)<20){
						if(vx_RightDeriv_peakDataAndPos[refined_rightPeakDeriv_vx.size()-1].number<vx_RightDeriv_peakDataAndPos[refined_rightPeakDeriv_vx.size()-2].number){
							rescue_iris_x_regionRight = vx_RightDeriv_peakDataAndPos[refined_rightPeakDeriv_vx.size()-1].number;	
						}else{
							rescue_iris_x_regionRight = vx_RightDeriv_peakDataAndPos[refined_rightPeakDeriv_vx.size()-2].number;	
			...
			rescue_iris_x_regionLeft = iris_x_regionLeft;
		}else if(leftGrads.size()!=0 && rightGrads.size()==0){		
			vector<int> refined_leftPeakDeriv_vx;
			PeakRefinement(leftPeakDeriv_vx , leftGrads	, refined_leftPeakDeriv_vx);
			if(refined_leftPeakDeriv_vx.size()==0){
				rescue_iris_x_regionLeft = 0;	
			}else if(refined_leftPeakDeriv_vx.size()==1){
				rescue_iris_x_regionLeft = refined_leftPeakDeriv_vx[0];
			}else{
				int firstHighest;
				int secondHighest;
				FindPeakof1D_Data* vx_LeftDeriv_peakDataAndPos = new FindPeakof1D_Data [refined_leftPeakDeriv_vx.size()];
				for(int i=0;i<refined_leftPeakDeriv_vx.size();++i){
					vx_LeftDeriv_peakDataAndPos[i].element = vec_Deriv_GPFv_x[refined_leftPeakDeriv_vx[i]];
					vx_LeftDeriv_peakDataAndPos[i].number = refined_leftPeakDeriv_vx[i];
				}
				sort(vx_LeftDeriv_peakDataAndPos , vx_LeftDeriv_peakDataAndPos + refined_leftPeakDeriv_vx.size());

				firstHighest = vx_LeftDeriv_peakDataAndPos[refined_leftPeakDeriv_vx.size()-1].element;
				secondHighest = vx_LeftDeriv_peakDataAndPos[refined_leftPeakDeriv_vx.size()-2].element;

				if(firstHighest>200 && secondHighest>200){
					if(vx_LeftDeriv_peakDataAndPos[refined_leftPeakDeriv_vx.size()-1].number < vx_LeftDeriv_peakDataAndPos[refined_leftPeakDeriv_vx.size()-2].number){
						rescue_iris_x_regionLeft = vx_LeftDeriv_peakDataAndPos[refined_leftPeakDeriv_vx.size()-2].number;
					}else{
						rescue_iris_x_regionLeft = vx_LeftDeriv_peakDataAndPos[refined_leftPeakDeriv_vx.size()-1].number;
					}
				}else{
					rescue_iris_x_regionLeft = vx_LeftDeriv_peakDataAndPos[refined_leftPeakDeriv_vx.size()-1].number;
				}		
	...
	irisMask_SideLength = (MIN(rescue_iris_x_regionRight - rescue_iris_x_regionLeft , iris_y_regionDown - iris_y_regionUp))/2.f*iris_GPF_rectSidePercent;//test

	//gpfCenter.x = (iris_x_regionRight+iris_x_regionLeft)/2;
	//gpfCenter.y = (iris_y_regionUp+iris_y_regionDown)/2;

	gpfCenter.x = (rescue_iris_x_regionRight+rescue_iris_x_regionLeft)/2;
	gpfCenter.y = (iris_y_regionUp+iris_y_regionDown)/2;

	circle(IrisContour ,  gpfCenter, irisMask_SideLength, Scalar::all(255) , CV_FILLED);


	if(thesisClipImage){			
		sprintf(thesisOutFileName , "%s\\EyelidDetection\\IrisContour_Intensity_preShift.jpg" , thesisImageOutDir);
		imwrite(thesisOutFileName , IrisContour);	
	}

	if(showDetail){
		imshow("RefinedGradX_EyeRegion" ,RefinedGradX_EyeRegion);
	}
	...
	if(thesisClipImage){					
		sprintf(thesisOutFileName , "%s\\EyelidDetection\\RefinedGradX_EyeRegion_Plus_IrisCenter.jpg" , thesisImageOutDir);
		imwrite(thesisOutFileName , RefinedGradX_EyeRegion);	
	}
...
inline void EyeInnerRegionGeneration(const Mat &Grad_X_Thresh_Pop , const Mat &Grad_Y_Thresh_Pop 
														...
														, Point &lineFilterCenter , int &rescue_iris_x_regionLeft, const bool &extremeRight
														, const bool &extremeLeft
														, int &rescue_iris_x_regionRight 	, const bool &extremeDown 
														, const bool &extremeUp
														, const Point &eyeCoarseCenter														
														, const vector<int> &leftPeakDeriv_vx , const vector<int> &rightPeakDeriv_vx
														, const vector<float> &vec_Deriv_GPFv_x
...
{
	Mat Grad_X_Y_And;	
	Mat Grad_X_Minus_And;
	Mat Grad_X_Y_And_Opening;
	Mat Grad_X_Y_And_Opening_Dila;
	Mat Grad_X_Y_And_Opening_Med;
	Mat FilterOutNoise_Grad_X;	
	...
	if(thesisClipImage){			
		sprintf(thesisOutFileName , "%s\\EyelidDetection\\BitWiseAnd_Grad_X_Y_AND.jpg" , thesisImageOutDir);
		imwrite(thesisOutFileName , Grad_X_Y_And);	

		sprintf(thesisOutFileName , "%s\\EyelidDetection\\Grad_X_Minus_Grad_X_Y_And.jpg" , thesisImageOutDir);
		imwrite(thesisOutFileName , Grad_X_Minus_And);	

		sprintf(thesisOutFileName , "%s\\EyelidDetection\\Grad_X_Minus_X_Y_And_Opening.jpg" , thesisImageOutDir);
		imwrite(thesisOutFileName , Grad_X_Y_And_Opening);	

		sprintf(thesisOutFileName , "%s\\EyelidDetection\\Grad_X_Minus_X_Y_And_Opening_Dila.jpg" , thesisImageOutDir);
		imwrite(thesisOutFileName , Grad_X_Y_And_Opening_Dila);	
	}
	...
	if(thesisClipImage){			
		sprintf(thesisOutFileName , "%s\\EyelidDetection\\BitWiseAnd_GauInv_X_Y_And_Opening_Dila.jpg" , thesisImageOutDir);
		imwrite(thesisOutFileName , FilterOutNoise_Grad_X);	
	}
	Morphology_Operations(FilterOutNoise_Grad_X, FilterOutNoise_Grad_X , MORPH_OPEN, 2,  MORPH_CROSS);		
	if(thesisClipImage){			
		sprintf(thesisOutFileName , "%s\\EyelidDetection\\FilterOutNoise_Grad_X_Open.jpg" , thesisImageOutDir);
		imwrite(thesisOutFileName , FilterOutNoise_Grad_X);	
	}
	Morphology_Operations(FilterOutNoise_Grad_X, FilterOutNoise_Grad_X , MORPH_ERODE, 2,  MORPH_CROSS);
	if(thesisClipImage){			
		sprintf(thesisOutFileName , "%s\\EyelidDetection\\FilterOutNoise_Grad_X_Open_Erosion.jpg" , thesisImageOutDir);
		imwrite(thesisOutFileName , FilterOutNoise_Grad_X);	
	}
	CenterCalculatUsingMoment(FilterOutNoise_Grad_X , lineFilterCenter.x , lineFilterCenter.y);	
	lineFilterCenter.x = (lineFilterCenter.x+eyeRegionCenter.x)/2;
	lineFilterCenter.y = (lineFilterCenter.y+eyeRegionCenter.y)/2;	


	if(thesisClipImage){			
		Mat OutLineFilterCenter = Grad_X_Y_And_Opening_Dila.clone();
		line(OutLineFilterCenter , Point(0 , lineFilterCenter.y) , Point(OutLineFilterCenter.cols - 1 , lineFilterCenter.y) , Scalar::all(255) , 1);
		line(OutLineFilterCenter , Point(lineFilterCenter.x , 0) , Point(lineFilterCenter.x , OutLineFilterCenter.rows - 1) , Scalar::all(255) , 1);
		sprintf(thesisOutFileName , "%s\\EyelidDetection\\lineFilterCenter.jpg" , thesisImageOutDir);
		imwrite(thesisOutFileName , OutLineFilterCenter);	
	}
	...
	LineFilterRefinement(contours , refined_contours , lineFilterCenter , RefinedGradX_EyeRegion 
									...
									, extremeRight	, rescue_iris_x_regionLeft	
									, extremeDown 	, rescue_iris_x_regionRight
									, extremeLeft
									, extremeUp
									, eyeCoarseCenter								
	...
	drawContours( ValleyPeakField, hull, 0, Scalar(255 , 255 , 255), CV_FILLED);//Fill the range in the convex hull	
	Morphology_Operations(ValleyPeakField, ValleyPeakField , MORPH_DILATE, 8,  MORPH_CROSS);		

	if(thesisClipImage){					
		sprintf(thesisOutFileName , "%s\\EyelidDetection\\ValleyPeakField.jpg" , thesisImageOutDir);
		imwrite(thesisOutFileName , ValleyPeakField);	
	}
}
...
{	
	...
	if(extremeUp){	
		...
		drawContours( IrisMask, hull, 0, Scalar(255 , 255 , 255), CV_FILLED);//Fill the range in the convex hull	
																			...
																			, const Point &eyeCoarseCenter)
{
	...
	//float shift_eyelidRegionLeft = vertexVerticalDist/8;
	//float shift_eyelidRegionRight = vertexVerticalDist/18;
	float shift_eyelidRegionLower = 15000/(vertexVerticalDist+eyeCoarseCenter.y);
	...
	if(getEyelidRegion)
		...
		IrisMask = Mat::zeros(Frame_wh.size() , CV_8UC1);
		return;
	}
	...
	if(thesisClipImage){	
		sprintf(thesisOutFileName , "%s\\StarBurst_Feature_Detection\\IrisMask_for_Limbus.jpg" , thesisImageOutDir);
		imwrite(thesisOutFileName , IrisMask);	
	}

	//Mat test = Frame_wh.clone();
	//for(int i=0;i<test.rows;++i){
	//	for(int j=0;j<test.cols;++j){
	//		if(EyelidRegionClipOutSclera.at<uchar>(i , j)==255){
	//			test.at<Vec3b>(i , j) = Vec3b::all(255);
	//		}
	//	}
	//}

	//IrisMask = EyelidRegionClipOutSclera.clone();
	//imshow("test" , test);
	//imshow("EyelidRegionClipOutSclera" , EyelidRegionClipOutSclera);
}
																 ...
																 , vector<float> &vec_Deriv_GPFv_x)
{
	...
	Mat Disp_GPFDeriv = Src.clone();
		
	Mat Display_GPFh_y = Mat::zeros(255 , Src.rows , CV_8UC3);
	Mat Display_GPFv_x = Mat::zeros(255 , Src.cols , CV_8UC3);

	Mat Display_DerivGPFh_y = Mat::zeros(255 , Src.rows , CV_8UC3);
	Mat Display_DerivGPFv_x = Mat::zeros(255 , Src.cols , CV_8UC3);
			
	//==========================Process Y First============================/
	//IPF
	reduce(Src, IPFh_y, 1, CV_REDUCE_AVG , CV_32FC1);//IPFh(y)		
	...
	//Derivative
	ConvertMatToVec1D(GPFh_y, vec_GPFh_y,true);
	...
	//==========================Process X Last============================/
	//IPF	
	reduce(Src_forX, IPFv_x, 0, CV_REDUCE_AVG , CV_32FC1);//IPFv(x)
	...
	//Derivative
	ConvertMatToVec1D(GPFv_x, vec_GPFv_x,false);	
	...
	//FindPeaks	
	FindPeaks_ver2(vec_Deriv_GPFv_x , find_vx_Derivpeak , 50);
	...
	if(showDetail){
		for( int i = 1; i < Display_DerivGPFh_y.cols; ++i ){
			if(i>vec_Deriv_GPFh_y.size()-1)break;
			line( Display_DerivGPFh_y, Point( (i-1), Display_DerivGPFh_y.rows - cvRound(vec_Deriv_GPFh_y[i-1] )) ,
							Point( (i), Display_DerivGPFh_y.rows - cvRound(vec_Deriv_GPFh_y[i]) ),
							Scalar(100 , 100 , 255), 2, 8, 0  );	
		}

		for(int i=0;i<find_hy_Derivpeak.size();++i){
			line(Display_DerivGPFh_y , Point(find_hy_Derivpeak[i] , 0) , Point(find_hy_Derivpeak[i] , Display_DerivGPFh_y.rows - 1) , Scalar::all(255));			
		}

	
	
		//Display	GPFh_y
		for(int i=0;i<vec_GPFh_y.size();++i){
			line(Display_GPFh_y, Point( (i-1), Display_GPFh_y.rows - cvRound(vec_GPFh_y[i-1] )) ,
							Point( (i), Display_GPFh_y.rows - cvRound(vec_GPFh_y[i]) ),
							Scalar(100 , 100 , 255), 2, 8, 0);	
		}

		//Display	GPFv_x
		for(int i=0;i<vec_GPFv_x.size();++i){
			line(Display_GPFv_x, Point( (i-1), Display_GPFv_x.rows - cvRound(vec_GPFv_x[i-1] )) ,
							Point( (i), Display_GPFv_x.rows - cvRound(vec_GPFv_x[i]) ),
							Scalar(100 , 100 , 255), 2, 8, 0);	
		}
	

		for( int i = 1; i < Display_DerivGPFv_x.cols; ++i ){
			if(i>vec_Deriv_GPFv_x.size()-1)break;
			line( Display_DerivGPFv_x, Point( (i-1), Display_DerivGPFv_x.rows - cvRound(vec_Deriv_GPFv_x[i-1]) ) ,
					Point( (i), Display_DerivGPFv_x.rows - cvRound(vec_Deriv_GPFv_x[i]) ),
					Scalar(100 , 100 , 255), 2, 8, 0  );			
		}
		...
		//Display Derivative Peak
		for(int i=0;i<2;++i){
			if(i==0){				
				line(Disp_GPFDeriv , Point(0 , iris_y_regionUp) , Point( Disp_GPFDeriv.cols-1, iris_y_regionUp) , Scalar::all(255));				
			}else{				
				line(Disp_GPFDeriv , Point(0 , iris_y_regionDown) , Point( Disp_GPFDeriv.cols-1, iris_y_regionDown) , Scalar::all(255));			
			}
		}


		for(int i=0;i<find_vx_Derivpeak.size();++i){
			line(Display_DerivGPFv_x , Point(find_vx_Derivpeak[i] , 0) , Point(find_vx_Derivpeak[i] , Display_DerivGPFv_x.rows - 1) , Scalar::all(255));	
			//line(Disp_GPFDeriv , Point(find_vx_Derivpeak[i] , 0) , Point( find_vx_Derivpeak[i], Disp_GPFDeriv.rows - 1) , Scalar::all(255));
		}

		for(int i=0;i<find_hy_peak.size();++i){
			line(Display_GPFh_y , Point(find_hy_peak[i] , 0) , Point(find_hy_peak[i] , Display_GPFh_y.rows - 1) , Scalar::all(255));	
			//line(Disp_GPFDeriv , Point(0 , find_hy_Derivpeak[i]) , Point( Disp_GPFDeriv.cols-1, find_hy_Derivpeak[i]) , Scalar::all(255));
		}


		imshow("Display_GPFh_y" , Display_GPFh_y);
		imshow("Display_GPFv_x" ,Display_GPFv_x);
		imshow("Disp_GPFDeriv" , Disp_GPFDeriv);
		imshow("Display_DerivGPFv_x" , Display_DerivGPFv_x);	
		imshow("Display_DerivGPFh_y" , Display_DerivGPFh_y);

		if(thesisClipImage){
			sprintf(thesisOutFileName , "%s\\IrisRegionExtraction_GPF\\Display_GPFh_y.jpg" , thesisImageOutDir);
			imwrite(thesisOutFileName , Display_GPFh_y);

			sprintf(thesisOutFileName , "%s\\IrisRegionExtraction_GPF\\Display_GPFv_x.jpg" , thesisImageOutDir);
			imwrite(thesisOutFileName , Display_GPFv_x);

			sprintf(thesisOutFileName , "%s\\IrisRegionExtraction_GPF\\GPF_Y_result.jpg" , thesisImageOutDir);
			imwrite(thesisOutFileName , Disp_GPFDeriv);

			sprintf(thesisOutFileName , "%s\\IrisRegionExtraction_GPF\\ROI_Y.jpg" , thesisImageOutDir);
			imwrite(thesisOutFileName , Src_forX);

			sprintf(thesisOutFileName , "%s\\IrisRegionExtraction_GPF\\Display_DerivGPFv_x.jpg" , thesisImageOutDir);
			imwrite(thesisOutFileName , Display_DerivGPFv_x);

			sprintf(thesisOutFileName , "%s\\IrisRegionExtraction_GPF\\Display_DerivGPFh_y.jpg" , thesisImageOutDir);
			imwrite(thesisOutFileName , Display_DerivGPFh_y);
...
inline bool InitialCoarseCenterCDF(const Mat &Src , Point &irisCoarseCenter){
	...
	reduce(Src_Hist, Hist_Sum, 0 ,  CV_REDUCE_SUM);//SUM
	...
	if(thesisClipImage){
		sprintf(thesisOutFileName , "%s\\InitialCoarseCenterCDF\\IrisMask_MaxCompon.jpg" , thesisImageOutDir);
		imwrite(thesisOutFileName , IrisContour);

		sprintf(thesisOutFileName , "%s\\InitialCoarseCenterCDF\\IrisMask_CDF.jpg" , thesisImageOutDir);
		imwrite(thesisOutFileName , Src_Binary);
	}

	if(showDetail){
		line(Src_Binary , Point(0 , irisCoarseCenter.y) , Point(Src_Binary.cols - 1 , irisCoarseCenter.y) , Scalar::all(255));
		line(Src_Binary , Point(irisCoarseCenter.x , 0) , Point(irisCoarseCenter.x , Src_Binary.rows - 1) , Scalar::all(255));

		imshow("CDFIrisContour" ,IrisContour);
		imshow("IrisMask_CDF" , Src_Binary);
	}
	return true;
}
...
inline void ClipFeaturePoints(const vector<Point> feature_point_Limbus , vector<Point> &clipped_feature_point_Limbus
											...
											, vector<Point> &refined_feture_point_Limbus)
{
	Mat ClipFeaturePointsDisp = Frame_wh.clone();	
	float dis_thresholdSoft_lowerEyelid_LimbusRefined = dis_thresholdSoft_lowerEyelid*1.2;
	float dis_thresholdSoft_upperEyelid_LimbusRefined = dis_thresholdSoft_upperEyelid*0.8;
	...
	for(int i=0;i<feature_point_Limbus.size();++i){
		...
		if(err_LowPar<dis_thresholdSoft_lowerEyelid_LimbusRefined){			
			if(err_UpPar<dis_thresholdSoft_upperEyelid_LimbusRefined){
				//Draw_Cross(ClipFeaturePointsDisp,feature_point_Limbus[i].x, feature_point_Limbus[i].y, 15 , 15 , Scalar(255 , 255 , 150));				
			}else{
				//Draw_Cross(ClipFeaturePointsDisp,feature_point_Limbus[i].x, feature_point_Limbus[i].y, 15 , 15 , Scalar(255 , 0 , 50));				
				lowerNoiseFts_Clipped.push_back(feature_point_Limbus[i]);
			}
		}else{			
			if(err_UpPar<dis_thresholdSoft_upperEyelid_LimbusRefined){
				//Draw_Cross(ClipFeaturePointsDisp,feature_point_Limbus[i].x, feature_point_Limbus[i].y, 15 , 15 , Scalar(255 , 0 , 150));				
				...
				if(getEyelidRegion){
					if(EyelidRegion.at<uchar>(feature_point_Limbus[i].y , feature_point_Limbus[i].x)==255){
						...
						refined_feture_point_Limbus.push_back(feature_point_Limbus[i]);
						Draw_Cross(ClipFeaturePointsDisp,feature_point_Limbus[i].x, feature_point_Limbus[i].y, 15 , 15 , Scalar(0 , 255 , 50));
					}
				}else{
					clipped_feature_point_Limbus.push_back(feature_point_Limbus[i]);
					refined_feture_point_Limbus.push_back(feature_point_Limbus[i]);
	...
	for(int i=0;i<lowerNoiseFts_Clipped.size();++i){				
		refined_feture_point_Limbus.push_back(lowerNoiseFts_Clipped[i]);
		Draw_Cross(ClipFeaturePointsDisp,lowerNoiseFts_Clipped[i].x, lowerNoiseFts_Clipped[i].y, 15 , 15 , Scalar(200 , 200 , 10));	
	}


	for(int i=0;i<upperNoiseFts_Clipped.size();++i){				
		refined_feture_point_Limbus.push_back(upperNoiseFts_Clipped[i]);
		Draw_Cross(ClipFeaturePointsDisp,upperNoiseFts_Clipped[i].x, upperNoiseFts_Clipped[i].y, 15 , 15 , Scalar(200 , 0 , 10));	
	}
	...
	//line(ClipFeaturePointsDisp , Point(iris_x_regionLeft , 0) , Point(iris_x_regionLeft , FRAMEH-1) , Scalar::all(255) , 1);
	//line(ClipFeaturePointsDisp , Point(iris_x_regionRight , 0) , Point(iris_x_regionRight , FRAMEH-1) , Scalar::all(255) , 1);
	imshow("ClipFeaturePointsDisp" , ClipFeaturePointsDisp);
	//waitKey(0);

	if(thesisClipImage){							
		sprintf(thesisOutFileName , "%s\\ClipFeaturePts\\ClipFeaturePointsDisp.jpg" , thesisImageOutDir);							
		imwrite(thesisOutFileName , ClipFeaturePointsDisp);	
	}	
}
																								...
																								, const float &eyeCoarseIrisCenter_ExtremeRegion_Left)
{
	...
	if(eyeRefinedIrisCenter.y<eyeCoarseIrisCenter_ExtremeRegion_Up){
		...
		if(showDetail){
			printf("extremeUp_Blink\n");//waitKey(0);
		}
	}	

	if(showDetail){
		line(DisplayExtremeRegion , Point(0 , eyeCoarseIrisCenter_ExtremeRegion_Down)
			, Point(FRAMEW-1 , eyeCoarseIrisCenter_ExtremeRegion_Down) , Scalar(100 , 200 , 200) , 1);
		line(DisplayExtremeRegion , Point(0 , eyeCoarseIrisCenter_ExtremeRegion_Up)
			, Point(FRAMEW-1 , eyeCoarseIrisCenter_ExtremeRegion_Up) , Scalar(100 , 200 , 200) , 1);
		line(DisplayExtremeRegion , Point(eyeCoarseIrisCenter_ExtremeRegion_Right , 0)
			, Point(eyeCoarseIrisCenter_ExtremeRegion_Right , FRAMEH-1) , Scalar(100 , 200 , 200) , 1);
		line(DisplayExtremeRegion , Point(eyeCoarseIrisCenter_ExtremeRegion_Left , 0)
			, Point(eyeCoarseIrisCenter_ExtremeRegion_Left , FRAMEH-1) , Scalar(100 , 200 , 200) , 1);

		imshow("DisplayExtremeRegion_Blink" , DisplayExtremeRegion);
	}
}
...
inline void DetermineWhetherEyeCoarseCenterInExtremeRegion(const Point &eyeCoarseIrisCenter
																								...
																								, const float &eyeCoarseIrisCenter_ExtremeRegion_Left)
{
	Mat DisplayExtremeRegionCoarseCenter = Frame_wh.clone();
	...
	if(showDetail){
		line(DisplayExtremeRegionCoarseCenter , Point(0 , eyeCoarseIrisCenter_ExtremeRegion_Down)
			, Point(FRAMEW-1 , eyeCoarseIrisCenter_ExtremeRegion_Down) , Scalar(100 , 200 , 200) , 2);
		line(DisplayExtremeRegionCoarseCenter , Point(0 , eyeCoarseIrisCenter_ExtremeRegion_Up)
			, Point(FRAMEW-1 , eyeCoarseIrisCenter_ExtremeRegion_Up) , Scalar(100 , 200 , 200) , 2);
		line(DisplayExtremeRegionCoarseCenter , Point(eyeCoarseIrisCenter_ExtremeRegion_Right , 0)
			, Point(eyeCoarseIrisCenter_ExtremeRegion_Right , FRAMEH-1) , Scalar(100 , 200 , 200) , 2);
		line(DisplayExtremeRegionCoarseCenter , Point(eyeCoarseIrisCenter_ExtremeRegion_Left , 0)
			, Point(eyeCoarseIrisCenter_ExtremeRegion_Left , FRAMEH-1) , Scalar(100 , 200 , 200) , 2);

		imshow("DisplayExtremeRegionCoarseCenter" , DisplayExtremeRegionCoarseCenter);		

		if(thesisClipImage){							
			sprintf(thesisOutFileName , "%s\\DisplayExtremeRegion.jpg" , thesisImageOutDir);
			imwrite(thesisOutFileName , DisplayExtremeRegionCoarseCenter);
		}
	}
}
...
{	
	...
	//-------------------High Frequency Component Removal---------------------//	
	Mat Gray_opening;	
	Mat Gray_openingGaussian;
	int openint_size = 19*((float)Frame_Gray.rows/FRAMEH);	
	int size_gaussian = 61*(Frame_Gray.rows/FRAMEH);
	if(size_gaussian%2==0)
		++size_gaussian;

	Morphology_Operations(Frame_Gray, Gray_opening , MORPH_OPEN, openint_size,  MORPH_RECT);		
	GaussianBlur( Gray_opening, Gray_openingGaussian, Size(size_gaussian,size_gaussian) , 0);			
	

	if(showDetail){
		imshow("Histogram_Eq" , Frame_Gray);
		imshow("Gray_openingGaussian" , Gray_openingGaussian);
	}
	if(thesisClipImage){
		sprintf(thesisOutFileName , "%s\\Histogram_Eq.jpg" , thesisImageOutDir);
		imwrite(thesisOutFileName , Frame_Gray);			
	}
	if(thesisClipImage){
		sprintf(thesisOutFileName , "%s\\Gray_opening.jpg" , thesisImageOutDir);
		imwrite(thesisOutFileName , Gray_opening);			
	}
	if(thesisClipImage){
		sprintf(thesisOutFileName , "%s\\Gray_openingGaussian.jpg" , thesisImageOutDir);
		imwrite(thesisOutFileName , Gray_openingGaussian);			
	}
	...
	Mat CoarseCenterDisp = Frame_wh.clone();	
	...
	if(caculateIris_Mask_done){
		getEyeCoarseCenter = MinimalIrisColorProcess(Iris_Mask , eyeCoarseCenter , size_gaussian 
																				...
																				, irisContour_size);		
		if(showDetail){
			imshow("IrisContour_byColor" , IrisContour_byColor);
		}

		if(thesisClipImage){
			Mat ThesisOutColorCoarseCenter = Frame_Gray.clone();
			line(ThesisOutColorCoarseCenter , Point(0 , eyeCoarseCenter.y) , Point(FRAMEW-1 , eyeCoarseCenter.y) , Scalar::all(255));
			line(ThesisOutColorCoarseCenter , Point(eyeCoarseCenter.x , 0) , Point(eyeCoarseCenter.x , FRAMEH-1) , Scalar::all(255));
			sprintf(thesisOutFileName , "%s\\MinimumIrisColor\\IrisMask_CoarseCenter.jpg" , thesisImageOutDir);
			imwrite(thesisOutFileName , ThesisOutColorCoarseCenter);	
		}

		if(eyeCoarseCenterLast.y>9/16.f*FRAMEH){		
		...
		if(thesisClipImage){	
			...
			line(ThesisOutColorCoarseCenter , Point(0 , eyeCoarseCenter.y) , Point(FRAMEW-1 , eyeCoarseCenter.y) , Scalar::all(255));
			line(ThesisOutColorCoarseCenter , Point(eyeCoarseCenter.x , 0) , Point(eyeCoarseCenter.x , FRAMEH-1) , Scalar::all(255));
			sprintf(thesisOutFileName , "%s\\InitialCoarseCenterCDF\\CDF_CoarseCenter.jpg" , thesisImageOutDir);
			imwrite(thesisOutFileName , ThesisOutColorCoarseCenter);			
		}
	}
	...
	if(getEyeCoarseCenter){
		...
		if(showDetail){
			line(CoarseCenterDisp , Point(0 , eyeCoarseCenter.y) , Point(FRAMEW-1 , eyeCoarseCenter.y) , Scalar::all(255));
			line(CoarseCenterDisp , Point(eyeCoarseCenter.x , 0) , Point(eyeCoarseCenter.x , FRAMEH-1) , Scalar::all(255));
			imshow("CoarseCenterDisp" , CoarseCenterDisp);
		}
	}else{
	...
	Mat Gray_openingGaussian_Gau;
	Mat Gray_openingGaussianOtsu;
	Mat Gray_openingGaussian_Gau_inv;		
	Point eyeRegionCenter;
	Point lineFilterCenter;
	int eyeRegionCenter_y_forEyelidDetection;
	int rescue_iris_x_regionLeft;
	int rescue_iris_x_regionRight;

	//Smooth
	GaussianBlur( Gray_openingGaussian, Gray_openingGaussian_Gau, Size(21 , 21) , 0);		
	threshold(Gray_openingGaussian_Gau, Gray_openingGaussianOtsu, 0, 255, CV_THRESH_BINARY | CV_THRESH_OTSU);
	Gray_openingGaussian_Gau_inv = Scalar::all(255) - Gray_openingGaussianOtsu;


	if(thesisClipImage){			
		sprintf(thesisOutFileName , "%s\\EyelidDetection\\Gray_openingGaussianOtsu.jpg" , thesisImageOutDir);
		imwrite(thesisOutFileName , Gray_openingGaussianOtsu);	

		sprintf(thesisOutFileName , "%s\\EyelidDetection\\Gray_openingGaussian_Gau_inv.jpg" , thesisImageOutDir);
		imwrite(thesisOutFileName , Gray_openingGaussian_Gau_inv);	
	...
	if(thesisClipImage){			
		sprintf(thesisOutFileName , "%s\\EyelidDetection\\Grad_X_Thresh_Pop.jpg" , thesisImageOutDir);
		imwrite(thesisOutFileName , Grad_X_Thresh_Pop);	

		sprintf(thesisOutFileName , "%s\\EyelidDetection\\Grad_Y_Thresh_Pop.jpg" , thesisImageOutDir);
		imwrite(thesisOutFileName , Grad_Y_Thresh_Pop);	
	...
	EyeInnerRegionGeneration(Grad_X_Thresh_Pop , Grad_Y_Thresh_Pop , eyeRegionCenter , ValleyPeakField
											...
											, rescue_iris_x_regionLeft , extremeRight , extremeLeft
											, rescue_iris_x_regionRight , extremeDown
											, extremeUp
											, eyeCoarseCenter												
											, leftPeakDeriv_vx , rightPeakDeriv_vx
											, vec_Deriv_GPFv_x
	...
	if(thesisClipImage){
		Mat OutIrisLeftRightRegion = Gray_openingGaussian.clone();
		line(OutIrisLeftRightRegion , Point(rescue_iris_x_regionLeft , 0) , Point(rescue_iris_x_regionLeft , OutIrisLeftRightRegion.rows - 1) , Scalar::all(255) , 1);
		line(OutIrisLeftRightRegion , Point(rescue_iris_x_regionRight , 0) , Point(rescue_iris_x_regionRight , OutIrisLeftRightRegion.rows - 1) , Scalar::all(255) , 1);
		sprintf(thesisOutFileName , "%s\\EyelidDetection\\IrisLeftRightRegion_rescue_x_left_right.jpg" , thesisImageOutDir);
		imwrite(thesisOutFileName , OutIrisLeftRightRegion);	
	}
	//Eyelid Detection
	...
	if(showDetail){
		imshow("ValleyPeakField" ,ValleyPeakField);	
	}
	if(printDebug){
		printf("\n 4\n");
	}
	...
	Mat inlinerLimbusFt_Parabola = Frame_wh.clone();	
	Mat EyelidRegion;
	...
	if(thesisClipImage){	
		Mat EyelidDetectionResult = Mat::zeros(480 , 640 , CV_8UC1);
		for(int j=0;j<EyelidDetectionResult.cols -1 ;++j){
			EyelidDetectionResult.at<uchar>(lowerParabolaTable[j] , j) = 255;
			EyelidDetectionResult.at<uchar>(upperParabolaTable[j] , j) = 255;
		}
		sprintf(thesisOutFileName , "%s\\EyelidDetection\\EyelidDetectionParabolaResult.jpg" , thesisImageOutDir);
		imwrite(thesisOutFileName , EyelidDetectionResult);	

		if(getEyelidRegion){
			sprintf(thesisOutFileName , "%s\\EyelidDetection\\EyelidRegion.jpg" , thesisImageOutDir);
			imwrite(thesisOutFileName , EyelidRegion);	
		}
	}

	if(showDetail){
		if(getEyelidRegion){
			imshow("EyelidRegion" , EyelidRegion);
		}
	}
	...
	Mat Iris_Mask_forLimbus;
	Mat Original_feature_point_Limbus = Frame_wh.clone();
	int final_irisRegion_Left;
	int final_irisRegion_Right;


	final_irisRegion_Left = rescue_iris_x_regionLeft;
	final_irisRegion_Right = rescue_iris_x_regionRight;
	...
	if(showDetail){
		for(int i=0;i<feature_point_Limbus.size();++i){
			Draw_Cross(Original_feature_point_Limbus , feature_point_Limbus[i].x , feature_point_Limbus[i].y , 15 , 15 , Scalar(0 , 255 , 10));
		}
		if(thesisClipImage){							
			sprintf(thesisOutFileName , "%s\\StarBurst_Feature_Detection\\Original_feature_point_Limbus.jpg" , thesisImageOutDir);							
			imwrite(thesisOutFileName , Original_feature_point_Limbus);	
		}				

		imshow("Iris_Mask_forLimbus" , Iris_Mask_forLimbus);
		imshow("Original_feature_point_Limbus" , Original_feature_point_Limbus);	
	}
	...
	Mat FeaturePoints_LimbusFtPts = Frame_wh.clone();														//For extreme down
	...
	Mat RefinedFeaturePts = Frame_wh.clone();
	
	
	//Use All of the feature_point_Limbus to estimate eyeRefinedIrisCenter
	ClipFeaturePoints(feature_point_Limbus , clipped_feature_point_Limbus
							, final_irisRegion_Right , final_irisRegion_Left
							...
							, refined_feture_point_Limbus);

	for(int i=0;i<refined_feture_point_Limbus.size();++i){
		Draw_Cross(RefinedFeaturePts , refined_feture_point_Limbus[i].x, refined_feture_point_Limbus[i].y, 15 , 15 , Scalar(0 , 255 , 255));
	}

	if(thesisClipImage){							
		sprintf(thesisOutFileName , "%s\\ClipFeaturePts\\RefinedFeaturePts.jpg" , thesisImageOutDir);							
		imwrite(thesisOutFileName , RefinedFeaturePts);	
	}	
	imshow("RefinedFeaturePts" , RefinedFeaturePts);

	//Convex Hull
	vector <vector<Point>>  hull(1);			
	convexHull( Mat(feature_point_Limbus), hull[0], false);		
	//convexHull( Mat(refined_feture_point_Limbus), hull[0], false);		
	drawContours( FeaturePoints_LimbusFtPtsConvexHull, hull, 0, Scalar(255 , 255 , 255), CV_FILLED);//Fill the range in the convex hull	


	if(thesisClipImage){							
		sprintf(thesisOutFileName , "%s\\ExactIrisCenter\\FeaturePoints_LimbusFtPtsConvexHull.jpg" , thesisImageOutDir);							
		imwrite(thesisOutFileName , FeaturePoints_LimbusFtPtsConvexHull);	
	}		
	...
	if(thesisClipImage){							
		sprintf(thesisOutFileName , "%s\\ExactIrisCenter\\MomentCenterByGrayLevel.jpg" , thesisImageOutDir);							
		imwrite(thesisOutFileName , MomentCenterByGrayLevel);	
	}	
	...
	//Testing//
	//eyeRefinedIrisCenter.y = (iris_y_regionUp+iris_y_regionDown)/2;
	//eyeRefinedIrisCenter.x = (final_irisRegion_Right+final_irisRegion_Left)/2;
		...
		if(frame_number%iris_maskModel_refreshFrame==0){				
			if(!bigmotionIrisCenter && getEyelidRegion){			
				if(EyelidRegion.at<uchar>(eyeRefinedIrisCenter.y , eyeRefinedIrisCenter.x)==255){				
					...
					Mat Refined_LimnusFtPtsConvexHull_Processed;
					vector<vector<Point> > contours;
					vector<Vec4i> hierarchy;		
					...
					findContours( Refined_LimnusFtPtsConvexHull_Processed, contours, hierarchy, CV_RETR_TREE, CV_CHAIN_APPROX_SIMPLE, Point(0, 0) );
					...
					if(contours.size()==0 || max_contourSize<50){						
						...
						if(thesisClipImage){							
							sprintf(thesisOutFileName , "%s\\IrisModelUpdating\\testROI.jpg" , thesisImageOutDir);							
							imwrite(thesisOutFileName , testROI);	
							sprintf(thesisOutFileName , "%s\\IrisModelUpdating\\Iris_ROI_forModel.jpg" , thesisImageOutDir);							
							imwrite(thesisOutFileName , Iris_ROI_forModel);	
						}	
					}
				}
			}
	...
	//-------------------Testing--------------------//	
	centerEstimation.push_back(eyeRefinedIrisCenter);	
	...
	//-----------------------------------------Display--------------------------------------------------//	
/*	Mat All_featureParabolaUpper = Mat::zeros(FRAMEH , FRAMEW , CV_8UC3);	
	Mat All_featureParabolaLower = Mat::zeros(FRAMEH , FRAMEW , CV_8UC3);	
	Mat ParabolaInliersOutliersUpper = Mat::zeros(FRAMEH , FRAMEW , CV_8UC3);	
	Mat ParabolaInliersOutliersLower = Mat::zeros(FRAMEH , FRAMEW , CV_8UC3);	*/	


	//------------------Display InliersOutliers----------------//
	/*DisplayInliersOutliersParabola(ParabolaInliersOutliersUpper ,upper_parabola_inlier , feature_point_upperEyelid 
												, upper_parabola_outlier , Frame , vertexParabolaUpper);
	DisplayInliersOutliersParabola(ParabolaInliersOutliersLower ,lower_parabola_inlier , feature_point_lowerEyelid
												, lower_parabola_outlier , Frame , vertexParabolaLower);	*/
	
	//------------------Display Feature-----------------//		
	//DisplayFeature(Frame , All_featureParabolaUpper , feature_point_upperEyelid);	
	//DisplayFeature(Frame , All_featureParabolaLower , feature_point_lowerEyelid);	
	

	//------------------Display Result-----------------//	
	if(thesisClipImage){	
		Mat Eyelid_Result = Mat::zeros(Frame_wh.size() , CV_8UC3);				
		DrawPrabolaVer3(Eyelid_Result  , upper_parabola_param);
		DrawPrabolaVer2(Eyelid_Result  , lower_parabola_param , Eyelid_Result);
		sprintf(thesisOutFileName , "%s\\EyelidDetection\\Eyelid_Result.jpg" , thesisImageOutDir);							
		imwrite(thesisOutFileName , Eyelid_Result);	
	}	
	...
	//Display Parabola
	//DrawPrabolaVer3(Gray_openingGaussian , upper_parabola_param);	
	//DrawPrabolaVer2(Gray_openingGaussian , lower_parabola_param , Gray_openingGaussian);	
	//DrawPrabolaVer2(Parabola , upper_parabola_param , Frame);	
	//DrawPrabolaVer2(Parabola , lower_parabola_param , Parabola);		

	DrawPrabolaVer3(EyePosition_Result  , upper_parabola_param);
	DrawPrabolaVer2(EyePosition_Result  , lower_parabola_param , EyePosition_Result);
	...
	//imshow("ParabolaInliersOutliersUpper" , ParabolaInliersOutliersUpper);
	//imshow("ParabolaInliersOutliersLower" , ParabolaInliersOutliersLower);		
	//imshow("All_featureParabolaUpper" , All_featureParabolaUpper);
	//imshow("All_featureParabolaLower" , All_featureParabolaLower);		
	
	
	//ParabolaInliersOutliersUpper.release();
	//ParabolaInliersOutliersLower.release();	
	//All_featureParabolaUpper.release();
	//All_featureParabolaLower.release();	
	


	

	//ParabolaInliersOutliersUpper.refcount = 0;
	//ParabolaInliersOutliersLower.refcount = 0;	
	//All_featureParabolaUpper.refcount = 0;
	//All_featureParabolaLower.refcount = 0;	
	...
	refined_feture_point_Limbus.clear();
...
inline void ScleraModelGeneration(const Mat &ScleraROI , Mat &Sclera_hist, int channels[] , int histSize[] , const float *ranges[]){    
	...
	cvtColor(ScleraROI, ScleraROI_HSV, CV_BGR2HSV);	
...
inline void IrisModelGeneration(const Mat &IrisROI , Mat &Iris_hist , int channels[] , int histSize[] , const float *ranges[]){    
	...
	cvtColor(IrisROI, IrisROI_HSV, CV_BGR2HSV);	
...
inline bool HistoModelValidTesting(const Mat &EyeImageForTestingIrisHistModel , const Mat &Iris_hist 
													...
													, int histSize[] , const int Iris_PerBin[] , ofstream &outFile)
{	
	Mat Iris_Mask;
	Mat Iris_Mask_Gau;
	Mat Iris_Mask_Otsu;
	...
	threshold(Iris_Mask_Gau, Iris_Mask_Otsu, 0, 255, CV_THRESH_BINARY | CV_THRESH_OTSU);
	...
	if(irisRate>irisRate_max && irisRate>iris_colorModelValidTestingIrisRate_initial){
		if(pixel_in_others==1){
			outFile<<"\\_1\\"<<irisRate<<endl;
			if(irisRate>iris_colorModelIrisRate_pixelInOthersOne*irisRegion_size){
				outFile<<"\\get_1!\\"<<iris_colorModelIrisRate_pixelInOthersOne*irisRegion_size<<endl;
			...
			if(thesisClipImage){							
				sprintf(thesisOutFileName , "%s\\IrisModelUpdating\\IrisRegionValidTesting.jpg" , thesisImageOutDir);							
				imwrite(thesisOutFileName , IrisRegionValidTesting);	
				sprintf(thesisOutFileName , "%s\\IrisModelUpdating\\Iris_Mask_BackProjected.jpg" , thesisImageOutDir);							
				imwrite(thesisOutFileName , Iris_Mask);	
				sprintf(thesisOutFileName , "%s\\IrisModelUpdating\\Iris_Mask_Otsu.jpg" , thesisImageOutDir);							
				imwrite(thesisOutFileName , Iris_Mask_Otsu);	
				sprintf(thesisOutFileName , "%s\\IrisModelUpdating\\EyeImageForTestingIrisHistModel.jpg" , thesisImageOutDir);							
				imwrite(thesisOutFileName , EyeImageForTestingIrisHistModel);	
			}	
		}
	}else{
		outFile<<"\\"<<irisRate<<endl;
		validHistModel = false;
	}

	//cout<<"========================================="<<endl;
	//cout<<"irisRate = "<<irisRate<<endl;
	//cout<<"irisRate_max = "<<irisRate_max<<endl;
	//cout<<"pixel_in_iris = "<<pixel_in_iris<<endl;
	//cout<<"pixel_in_others = "<<pixel_in_others<<endl;
	//cout<<"iris_colorModelIrisRate_pixelInOthersOne*irisRegion_size = "<<iris_colorModelIrisRate_pixelInOthersOne*irisRegion_size<<endl;
...
inline void IrisModelHandeling(bool &gotIrisROI , bool &irisDynamicMaskGeneration , bool &readIrisModel
								  ...
								  , const Mat &IrisRegionValidTesting
								  , bool &caculateIris_Mask_done , int &countRefreshTimes , ofstream &outFile)
{
	...
	if(checkIrisColorModel){	
		irisHist_IsValidModel = HistoModelValidTesting(EyeImageForTestingIrisHistModel , Iris_hist_cal
																			, IrisRegionValidTesting , histSize , Iris_PerBin , outFile);
...
inline void WriteOutTestingData(const char *testVarianceWriteOutFileDir){
	...
	sprintf(centerEstimation_convexHullFineCenter_fileName , "%s\\centerEstimation_convexHullFineCenter.txt" , testVarianceWriteOutFileDir);
												...
												, const bool &y_inFirstORLastRows)											
{
	...
	circle(Scene_cal , Point(posX , posY) , MIN(x_dist , y_dist)/2-5 , Scalar(150 , 200 , 50) , CV_FILLED);
...
inline bool CalibrationProcedureWhole(Mat &Scene_chessboard , Mat &Scene_calibration , int &count_calProcedure , double &time_cal
														...
														, const int &numXSpace , const int &numYSpace
														, int &countEyePut														
														, vector<vector<Point>> &testEyeInputPts
														, vector<Point> &testOneDstEyePts
														, vector<Point> &calibrationPatternPtsStep_One
														, int &countCalibrationPatternPtsStep_One_pos)
{
	...
	if(count_calProcedure==0){
		...
		testEyeInputPts.clear();
		testOneDstEyePts.clear();
...
#pragma endregion
			...
			testEyeInputPts.push_back(testOneDstEyePts);
			testOneDstEyePts.clear();
		}
		...
		if(calibrationPts_space==calibrationPattern::Step_space_two){
			...
			if(countCalibrationPatternPtsStep_One_pos>calibrationPatternPtsStep_One.size()-1){
				...
				circle(Scene_calibration , calibrationPatternPtsStep_One[countCalibrationPatternPtsStep_One_pos] , MIN(x_dist , y_dist)/2-5 , Scalar(150 , 200 , 50) , CV_FILLED);
	...
	if((calibratedChessBoardPtsLast==calibratedChessBoardPts) && getCalibrationPoints && (eyeState == Eyeopen )
		&& setEyeCornerAndEyePosReady){		
			if(countEyePut>5){
				testOneDstEyePts.push_back(eyeRefinedIrisCenter);
	...
	if(calibration_CollectDataDone){	
		...
		for(int i=0;i<calibratedCalPoints.size();++i){
			circle(ChessBoardThesisPic , calibratedCalPoints[i] , MIN(x_dist , y_dist)/2-5 , Scalar(150 , 200 , 50) , CV_FILLED);
...
#pragma region
	if(pressO){
		Mat Testing = Mat::zeros(480 , 640 , CV_8UC3);
		Mat Testing_GazeRegion = Mat::zeros(MonitorH , MonitorW , CV_8UC3);

		for(int i=0;i<calibratedEyeRefinedCenter.size();++i){
			Draw_Cross(Testing , calibratedEyeRefinedCenter[i].x , calibratedEyeRefinedCenter[i].y , 15 , 15 , Scalar(20 , 255 , 255) , 2);
		}
		for(int i=0;i<calibratedCalPoints.size();++i){
			Draw_Cross(Testing_GazeRegion , calibratedCalPoints[i].x , calibratedCalPoints[i].y , 15 , 15 , Scalar(255 , 255 , 255) , 2);
		}

		for(int i=0;i<Testing.rows;++i){
			for(int j=0;j<Testing.cols;++j){
				if(mappingSliceMap[mapIndex].returnMap().at<uchar>(i , j)==255){
					Testing.at<Vec3b>(i , j) = Vec3b(255 , 255 , 255);
				}
			}
		}
		...
		Draw_Cross(Testing , eyePosPoint.x , eyePosPoint.y , 15 , 15 , Scalar(10 , 10 , 255) , 2);

		Draw_Cross(Testing , right_InterCalPts_eyeRegion.x , right_InterCalPts_eyeRegion.y , 15 , 15 , Scalar(255 , 10 , 255) , 2);
		Draw_Cross(Testing , left_InterCalPts_eyeRegion.x , left_InterCalPts_eyeRegion.y , 15 , 15 , Scalar(255 , 10 , 255) , 2);
		Draw_Cross(Testing , up_InterCalPts_eyeRegion.x , up_InterCalPts_eyeRegion.y , 15 , 15 , Scalar(255 , 10 , 255) , 2);
		Draw_Cross(Testing , down_InterCalPts_eyeRegion.x , down_InterCalPts_eyeRegion.y , 15 , 15 , Scalar(255 , 10 , 255) , 2);

		Draw_Cross(Testing_GazeRegion , right_InterCalPts_gazeRegion.x , right_InterCalPts_gazeRegion.y , 15 , 15 , Scalar(100 , 10 , 255) , 2);
		Draw_Cross(Testing_GazeRegion , left_InterCalPts_gazeRegion.x , left_InterCalPts_gazeRegion.y , 15 , 15 , Scalar(100 , 10 , 255) , 2);
		Draw_Cross(Testing_GazeRegion , up_InterCalPts_gazeRegion.x , up_InterCalPts_gazeRegion.y , 15 , 15 , Scalar(100 , 10 , 255) , 2);
		Draw_Cross(Testing_GazeRegion , down_InterCalPts_gazeRegion.x , down_InterCalPts_gazeRegion.y , 15 , 15 , Scalar(100 , 10 , 255) , 2);


		imwrite(".//Testing_GazeRegion.jpg" , Testing_GazeRegion);
		imwrite(".//Testing_EyeRegion.jpg" , Testing );
		imshow("Testing_EyeRegion" , Testing);
		imshow("Testing_GazeRegion" , Testing_GazeRegion);
	}
#pragma endregion
...
#pragma region
	if(pressO){
		...
		Draw_Cross(Testing , mappingSliceMap[mapIndex].returnElement("left").first.x , mappingSliceMap[mapIndex].returnElement("left").first.y , 15 , 15 , Scalar(255 , 10 , 255) , 2);
		Draw_Cross(Testing , mappingSliceMap[mapIndex].returnElement("up").first.x , mappingSliceMap[mapIndex].returnElement("up").first.y , 15 , 15 , Scalar(255 , 10 , 255) , 2);
		Draw_Cross(Testing , mappingSliceMap[mapIndex].returnElement("right").first.x , mappingSliceMap[mapIndex].returnElement("right").first.y , 15 , 15 , Scalar(255 , 10 , 255) , 2);
		Draw_Cross(Testing , mappingSliceMap[mapIndex].returnElement("down").first.x , mappingSliceMap[mapIndex].returnElement("down").first.y , 15 , 15 , Scalar(255 , 10 , 255) , 2);
		Draw_Cross(Testing , mappingSliceMap[mapIndex].returnCenter().x , mappingSliceMap[mapIndex].returnCenter().y , 15 , 15 , Scalar(255 , 10 , 255) , 2);	
		imshow("Testing" , Testing);	
		imwrite(".//TestingeyePos_Map.jpg", Testing);	



		ofstream outfile(".//outTestHomography.txt");
		outfile<<"mappingEyePts = "<<endl;
		outfile<<"left = "<<mappingSliceMap[mapIndex].returnElement("left").first<<endl;
		outfile<<"right = "<<mappingSliceMap[mapIndex].returnElement("right").first<<endl;
		outfile<<"up = "<<mappingSliceMap[mapIndex].returnElement("up").first<<endl;
		outfile<<"down = "<<mappingSliceMap[mapIndex].returnElement("down").first<<endl;
		outfile<<"mappingCalPts = "<<endl;
		outfile<<"left = "<<calibratedCalPoints[mappingSliceMap[mapIndex].returnElement("left").second]<<endl;
		outfile<<"right = "<<calibratedCalPoints[mappingSliceMap[mapIndex].returnElement("right").second]<<endl;
		outfile<<"up = "<<calibratedCalPoints[mappingSliceMap[mapIndex].returnElement("up").second]<<endl;
		outfile<<"down = "<<calibratedCalPoints[mappingSliceMap[mapIndex].returnElement("down").second]<<endl;
		outfile<<"Eye original = "<<endl<<EyeOriginal<<endl;
		outfile<<"SceneMat = "<<endl<<SceneMat<<endl;
		outfile<<"Homography Matrix = "<<endl<<HompgraphyMatrix<<endl;
		outfile<<"gazePoint = "<<endl<<gazePoint;

		outfile<<"--------------------------"<<endl;
		for(int i=0;i<mappingSliceMap.size();++i){
			float dist = DistanceCaculateEuclidean(mappingSliceMap[i].returnCenter() , eyePosPoint);			
			outfile<<"i = "<<i<<" , dist = "<<dist<<endl;
		}
		outfile<<"---------min = "<<mapIndex<<endl;
	}
#pragma endregion
}
...
{	
	...
	for(int i=0;i<calInterAnchorPoints.size();++i){
		...
		if(calBrationMethod==calibrationMethod::BilinearInterpolation || calBrationMethod==calibrationMethod::BicubicInterpolation){
			sliceMap.reorganize();
		...
		/*if(calBrationMethod==calibrationMethod::BilinearInterpolation || calBrationMethod==calibrationMethod::BicubicInterpolation
			|| calBrationMethod==calibrationMethod::HomographySliceMapping){
			sliceMap.centerConstruction();			
		}*/
										 ...
										 , bool &realGazeInput)
{
	if(countInEyeGazeTestingFunction==0){		
		...
		for(int i=0;i<numYTestPts;++i){
			for(int j=0;j<numXTestPts;++j){
				...
				circle(TestAllPtsImg , tmp , MIN(x_distTest , y_distTest)/2-5 , Scalar(150 , 200 , 50) , CV_FILLED);
	...
	if((time_duringCal==testInterTimePoints && time_duringCal != time_duringCalLast) || countInEyeGazeTestingFunction==0){
		...
		circle(Scene_gazetest , dispTestPts , MIN(x_distTest , y_distTest)/2-5 , Scalar(150 , 200 , 50) , CV_FILLED);
...
int main(int argc , char *argv[]){	
		...
		for(int i=1;i<argc;++i){
			...
			}else if(!strcmp(argv[i] , "-erl")){
				irisCenter_Intensity_Color_ExchangeRegionLimit = (int)atof(argv[i+1]);
			}
		}
		...
		ReadCameraCalibrationCoeff(CameraMatrix_Eye , DistCoeffs_Eye , ".//outCameraCoeff_Eye2.txt");
		ReadCameraCalibrationCoeff(CameraMatrix_Scene , DistCoeffs_Scene , ".//outCameraCoeff_Scene.txt");

		
		//===========Output File==========/		
		char calibrationNumTimesName[MAX_WORD_LEN] = "CalibrationNum_19";			
		char eyePosCenter_GroundTruthFile[MAX_WORD_LEN];
		char testVarianceWriteOutFileDir[MAX_WORD_LEN];
		char* testVarianceWriteOutDir	= "F:\\LAB_new\\LAB\\Eye_track_code\\Eye_Pos_Track\\Eye_Pos_Track_Algori_2_Modification\\Eye_Pos_Track_Algori_2_Modification\\TestVarianceData";
		...
		//VideoCapture cap(3);
		VideoCapture cap(1);
		VideoCapture cap_scene(2);
		//VideoCapture cap("F:\\LAB_new\\LAB\\eye_frame\\eye_frame_source\\NoIRlight\\med_down\\med_down_22.avi");


		//===========Input/Output File Set==========/
		//Input Grouund Truth File Name
		char inputFile_dir [MAX_WORD_LEN] = "F:\\LAB_new\\LAB\\eye_frame\\eye_frame_source\\NoIRlight\\med_down";
		char inputFile [MAX_WORD_LEN]= "med_down_11";
		char inputFileName [MAX_WORD_LEN];
		sprintf(inputFileName , "%s\\%s.txt" , inputFile_dir , inputFile);		
		//ReadGroundTruth(groundTruth , inputFileName);

		
		//Write Out GroundTruth File Name
		//char eyeCenterGroundTruthOutputFile_name[MAX_WORD_LEN] = "med_down_9";
		//sprintf(eyePosCenter_GroundTruthFile , "%s\\%s.txt" ,vedioWriteOutDir , eyeCenterGroundTruthOutputFile_name);

		//Test Variance File Dir		
		//sprintf(testVarianceWriteOutFileDir , "%s\\%s" , testVarianceWriteOutDir , videoOutputFile_name);
		//sprintf(testVarianceWriteOutFileDir , "%s" , inputFile_dir);


				
		namedWindow("EyePosition_CenterResult" , CV_WINDOW_AUTOSIZE);		
		namedWindow("Scene" , CV_WINDOW_FULLSCREEN);		
		resizeWindow("Scene",MonitorW,MonitorH);//col , row
		setWindowProperty("Scene",CV_WND_PROP_OPENGL, CV_WINDOW_FULLSCREEN);
		...
		sprintf(IrisHisto_output_fileName ,".\\IrisHistoModel3D.txt");	
		ofstream outFile(".//Imro.txt");
		while(cap.read(Frame) && cap_scene.read(Scene_image)){
			...
			if(thesisClipImage){
				sprintf(thesisOutFileName , "%s\\Frame_wh.jpg" , thesisImageOutDir);
				imwrite(thesisOutFileName , Frame_wh);
				sprintf(thesisOutFileName , "%s\\Frame.jpg" , thesisImageOutDir);
				imwrite(thesisOutFileName , Frame);
			}
			...
			if(thesisClipImage){
				sprintf(thesisOutFileName , "%s\\Setup.jpg" , thesisImageOutDir);
				imwrite(thesisOutFileName , Frame_wh);			
			}
			//============Check Whether get Iris ROI============/
			//if( selectObjectIris && selection.width > 0 && selection.height > 0 ){					
			//	Mat Iris_ROI = Frame_wh(selection);				
			//	Iris_ROI_forModel = Iris_ROI.clone();
			//	bitwise_not(Iris_ROI, Iris_ROI);
			//}//Can be removed in the last version of system

			
			double t = (double)getTickCount();  			
			if(setEyeCornerAndEyePosReady){				
				//============Convert Color Space============/
				cvtColor(Frame_wh , Frame_Gray , CV_BGRA2GRAY);		
				
				

				if(thesisClipImage){
					sprintf(thesisOutFileName , "%s\\Frame_Gray.jpg" , thesisImageOutDir);
					imwrite(thesisOutFileName , Frame_Gray);			
				}
				...
				if(eyeState==Eyeopen){
					...
					switch(calBrationMethod){
						case calibrationMethod::BilinearInterpolation:{
									...
									break;
						case calibrationMethod::BicubicInterpolation:{
					...
					if(!eyeGazeTestProcedure && pressO){
						imwrite(".//Gaze.jpg" , Scene_image);
					}
				}			
			}
...
#pragma region			
					...
					char outPutFileDir[MAX_WORD_LEN];		
					char test_imageFille[MAX_WORD_LEN];		
					Mat EyePtsMat = Mat::zeros(480 , 640 , CV_8UC3);

					sprintf(tmpOutFileName , "%s\\testPts.txt" , analysisGazeOutputDir);
					ofstream file_out(tmpOutFileName);
					file_out<<"getSlicingMap = "<<getSlicingMap<<endl;
					file_out<<"calibrationMethod = "<<calBrationMethod<<endl;
					file_out<<"calibration size = "<<calibratedCalPoints.size()<<endl;
					file_out<<"cal , eye : "<<endl;
					for(int i=0;i<calibratedCalPoints.size();++i){
						file_out<<calibratedCalPoints[i]<<" , "<<calibratedEyeRefinedCenter[i]<<endl;
						Draw_Cross(EyePtsMat , calibratedEyeRefinedCenter[i].x , calibratedEyeRefinedCenter[i].y , 15 , 15 , Scalar(0 , 255 , 0) , 2);
					}
					sprintf(test_imageFille , "%s\\EyeCenterPts.jpg" ,analysisGazeOutputDir);
					imwrite(test_imageFille, EyePtsMat);			


					if(calibrationPts_space==calibrationPattern::Step_space_two && getSlicingMap){
						file_out<<"anchor pts : "<<endl;
						file_out<<"calInterAnchorPoints.size = "<<calInterAnchorPoints.size()<<endl;
						file_out<<endl;
						for(int i=0;i<calInterAnchorPoints.size();++i){
							file_out<<calInterAnchorPoints[i]<<endl;
						}
					}
					file_out<<"chessBoardSize = "<<numXSpace<<" X "<<numYSpace<<endl;
					file_out<<"x_dist = "<<x_dist<<endl;
					file_out<<"y_dist = "<<y_dist<<endl;					
	
					
					if(calibrationPts_space==calibrationPattern::Step_space_two && getSlicingMap){
						Mat Testing_AllSliceInOne = Mat::zeros(480 , 640 , CV_8UC3);
						for(int i=0;i<mappingSliceMap.size();++i){
							sprintf(test_imageFille , "%s\\slicingMap_%d.jpg" ,analysisGazeOutputDir , i+1);
							Mat Testing = Mat::zeros(480 , 640 , CV_8UC3);
							for(int k=0;k<Testing.rows;++k){
								for(int l=0;l<Testing.cols;++l){
									if(mappingSliceMap[i].returnMap().at<uchar>(k , l)==255){
										Testing.at<Vec3b>(k , l) = Vec3b::all(255);
										Testing_AllSliceInOne.at<Vec3b>(k , l) = Vec3b::all(255);
									}
								}
							}
							Point sliceMapCenter = mappingSliceMap[i].returnCenter();
							Draw_Cross(Testing , sliceMapCenter.x , sliceMapCenter.y , 15 , 15 , Scalar(0 , 255 , 255) , 2);							
							imwrite(test_imageFille, Testing);								
						}							
						for(int i=0;i<calibratedEyeRefinedCenter.size();++i){
							Draw_Cross(Testing_AllSliceInOne , calibratedEyeRefinedCenter[i].x , calibratedEyeRefinedCenter[i].y , 15 , 15 , Scalar(0 , 255 , 0) , 2);	
						}
						sprintf(test_imageFille , "%s\\slicingMapAll.jpg" ,analysisGazeOutputDir);
						imwrite(test_imageFille, Testing_AllSliceInOne);			
					}
					


					for(int i=0;i<testEyeInputPts.size();++i){
						sprintf(test_imageFille , "%s\\EyePtsMedianImage_%d.jpg" ,analysisGazeOutputDir , i);
						Mat TestEyeMat = Mat::zeros(480 , 640 , CV_8UC3);
						for(int j=0;j<testEyeInputPts[i].size();++j){
							Draw_Cross(TestEyeMat, testEyeInputPts[i][j].x , testEyeInputPts[i][j].y , 30 , 30 , Scalar(10 , 255 , 255) , 2)	;
						}			
						imwrite(test_imageFille, TestEyeMat);	
					}
#pragma endregion
					...
					switch(calBrationMethod){
						...
						case calibrationMethod::Polynomial_Order:{										
									calibrationProcedureDone = MappingEyeGaze_PolyNomialALLPtsCaculated(calibratedEyeRefinedCenter 
																																						...
																																						, meanSquareError);	
#pragma region
										//Output file of mapping coefficient
										char outPutfiIleName[300];
										sprintf(outPutfiIleName , "%s\\MappingCoeff.txt" , analysisGazeOutputDir);
										ofstream fout(outPutfiIleName);
										fout<<"mappingX:"<<endl;
										for(int i=0;i<numberOfVar;++i){
											fout<<" "<<mapping_paramOptX[i];
										}
										fout<<endl;
										fout<<"mappingY:"<<endl;
										for(int i=0;i<numberOfVar;++i){
											fout<<" "<<mapping_paramOptY[i];
										}
										fout<<endl;
										fout<<"EyePtsTransformMat_Opt = "<<endl;
										for(int i=0;i<EyePtsTransformMat_Opt.rows;++i){
											for(int j=0;j<EyePtsTransformMat_Opt.cols;++j){
												fout<<EyePtsTransformMat_Opt.at<double>(i , j)<<" ";
											}
											fout<<endl;
										}
										fout<<"ScenePtsTransformMat_Opt = "<<endl;
										for(int i=0;i<ScenePtsTransformMat_Opt.rows;++i){
											for(int j=0;j<ScenePtsTransformMat_Opt.cols;++j){
												fout<<ScenePtsTransformMat_Opt.at<double>(i , j)<<" ";
											}
											fout<<endl;
										}
#pragma endregion
						...
						case calibrationMethod::Polynomial_All:{
									...
									char orderFileName[MAX_WORD_LEN];
									sprintf(orderFileName , "%s\\PolyOrder_opt.txt"  , analysisGazeOutputDir);
									ofstream fileOut(orderFileName);
												  fileOut<<"orderOpt = "<<orderOpt<<endl;
												  fileOut<<"numberOfVar = "<<numberOfVar<<endl;
												  fileOut<<endl;
												  fileOut<<"mapping_paramOptX = "<<endl;
												  for(int j=0;j<numberOfVar;++j){
													 fileOut<<mapping_paramOptX[j]<<" ";
												  }
												  fileOut<<endl;
												  fileOut<<"mapping_paramOptY = "<<endl;
												  for(int j=0;j<numberOfVar;++j){
													fileOut<<mapping_paramOptY[j]<<" ";
												  }
												  fileOut<<endl;
												  fileOut<<"EyePtsTransformMat_Opt = "<<endl;
												  for(int j=0;j<EyePtsTransformMat_Opt.rows;++j){
													for(int k=0;k<EyePtsTransformMat_Opt.cols;++k){
														fileOut<<EyePtsTransformMat_Opt.at<double>(j , k)<<" ";
													}
													fileOut<<endl;
												  }
												  fileOut<<endl;
												  fileOut<<"ScenePtsTransformMat_Opt = "<<endl;
												  for(int j=0;j<ScenePtsTransformMat_Opt.rows;++j){
													for(int k=0;k<ScenePtsTransformMat_Opt.cols;++k){
														fileOut<<ScenePtsTransformMat_Opt.at<double>(j , k)<<" ";
													}
													fileOut<<endl;
												  }												 
			...
			//sprintf(s , "Non Voluntary = %d" ,non_voluntaryBlinkCount);
			//putText(EyePosition_Result
			//			, s
			//			, Point(Frame.cols - 400 , Frame.rows - 75)
			//			, FONT_HERSHEY_PLAIN
			//			, 1.5/480.0*Frame.cols
			//			, Scalar(255, 255 , 10)
			//			, 4
			...
			//sprintf(s , "Total Count = %d" ,count_sum_forTest);
			//putText(EyePosition_Result
			//			, s
			//			, Point(Frame.cols - 600 , Frame.rows - 25)
			//			, FONT_HERSHEY_PLAIN
			//			, 1/480.0*Frame.cols
			//			, Scalar(255, 255 , 10)
			//			, 2
		...
		/*	}else if(eyeGazeTestProcedure){
			...
			if(setEyeCornerAndEyePosReady){
				...
				if(thesisClipImage){							
					sprintf(thesisOutFileName , "%s\\ExactIrisCenter\\EyePosition_CenterResult.jpg" , thesisImageOutDir);							
					imwrite(thesisOutFileName , EyePosition_CenterResult);	
				}	
			}else{
			...
			if(key==32/*Space*/){
				...
				}else if(enterstring=="clip"){
					printf("\n Take a shot!\n");
					thesisClipImage = true;
				}
			...
			Frame.refcount = 0;
			Frame_Gray.refcount = 0;
			Histogram_Eq.refcount = 0;
			Frame_wh.refcount = 0;		
		...
		//======Testing Variance======/
		//WriteOutTestingData(testVarianceWriteOutFileDir);
		//WriteOutEyeCenterGroundTruthData(eyePosCenter_GroundTruthFile);



		//======Caculate result======/
		avg_fps/=frame_number;
		cout<<"frame_number = "<<frame_number<<endl;
		cout<<"avg_fps = "<<avg_fps<<endl;
